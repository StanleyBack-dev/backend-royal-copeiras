...
export class FreezeBudgetPdfService {
  ...
  async execute(userId: string, input: FreezeBudgetPdfInputDto) {
    ...
    if (budget.pdfFrozenAt) {
      throw AppException.from(APP_ERRORS.budgets.alreadyFrozen, undefined);
    }
    ...
    await this.budgetsRepository.update(input.idBudgets, {
      pdfSnapshot: snapshot,
      pdfHash: snapshotHash,
      pdfFrozenAt: frozenAt,
    });
