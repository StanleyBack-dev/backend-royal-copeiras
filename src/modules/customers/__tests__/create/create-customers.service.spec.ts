...
describe("CreateCustomersService", () => {
  ...
  beforeAll(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [{ provide: CreateCustomersService, useValue: serviceMock }],
    }).compile();
  ...
  it("should throw error for duplicate document", async () => {
    (service.execute as jest.Mock).mockRejectedValueOnce(
      new Error("Já existe um cliente com este documento."),
    );
    const input = {
      name: "Cliente Exemplo",
      document: "12345678901",
    ...
    await expect(service.execute("user-id-test", input)).rejects.toThrow(
