...
describe("UpdateCustomersService", () => {
  ...
  beforeAll(async () => {
    const serviceMock = {
      execute: jest
        .fn()
        .mockResolvedValue({ ...customerMock, name: "Novo Nome" }),
    };
    const module: TestingModule = await Test.createTestingModule({
      providers: [{ provide: UpdateCustomersService, useValue: serviceMock }],
    }).compile();
  ...
  it("should throw error if customer not found", async () => {
    ...
    await expect(
      service.execute("user-id-test", { idCustomers: "not-exist" }),
